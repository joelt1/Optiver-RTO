...
SET_VOLUME = 2
BIG_VOL = 3
BIG_PRESSURE = -2 # Pressure value for BIG_VOL to occur
MIN_PRESSURE = -2 # Negative value allows us to take advantage of large spreads in the market when safe
MAX_PRESSURE = 100
...
class AutoTrader(BaseAutoTrader):
    def __init__(self, loop: asyncio.AbstractEventLoop):
        ...
        self.bid_count = 1
        self.bid_acceptance = 0
        self.ask_count = 1
        self.ask_acceptance = 0
        ...
        try:
            if instrument == Instrument.FUTURE:
                ...
                if bid_prices[0] != 0 and ask_prices[0] != 0 and abs(self.etf_position) < DUMP_POSITION:
                    ...
                    bid_price = intval(fair_value - spread + PRESSURE_SPREAD*self.bid_pressure - RETURN_STRENGTH*self.etf_position)
                    ask_price = intval(fair_value + spread - PRESSURE_SPREAD*self.ask_pressure - RETURN_STRENGTH*self.etf_position)
                    flag = "flag 2"
                    if self.count % 1 == 0:
                        string = "{}, Bid Acceptance {:.2f}, Ask Acceptance {:.2f}, Spread {}, Bid Pressure {}, Ask Pressure {}, Position {}".format(self.count, (self.bid_acceptance/self.bid_count), (self.ask_acceptance/self.ask_count), ask_price - bid_price, self.bid_pressure, self.ask_pressure, self.etf_position)
                        self.log(string)
                    self.count += 1
