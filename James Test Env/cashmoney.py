...
GAUSSIAN_SHIFT = 1
Z_SCORE_SIGMA = 3
R_SQUARED_THRESH = 0.2
BUY_SELL_DIFF_THRESH = 15
...
class AutoTrader(BaseAutoTrader):
    ...
    def ideal_trade_volume(data):
        ...
        if np.sqrt(np.var(data)) == 0:
            ...
            if z_score < GAUSSIAN_SHIFT:
                ...
                return int(round(V_MAX * np.exp(-(1 / 2) * (z_score + GAUSSIAN_SHIFT) ** 2)))
        ...
        # Cornering market case
            ...
            for order_id in self.active_orders:
                if len(self.active_orders[order_id]) != 3:
                    can_cancel = False
                elif len(self.active_orders[order_id]) == 3 and len(set(self.active_orders[order_id])) > 1:
                    can_cancel = False
            if can_cancel:
                for order_id in self.active_orders:
                    self.send_cancel_order(order_id)
    ...
    def on_order_status_message(self, client_order_id: int, fill_volume: int, remaining_volume: int, fees: int) -> None:
        ...
        try:
            ...
            if len(self.active_orders[client_order_id]) == 4:
